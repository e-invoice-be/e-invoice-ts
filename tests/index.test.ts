...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['E_INVOICE_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: EInvoiceAPI) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new EInvoiceAPI({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new EInvoiceAPI({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new EInvoiceAPI({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['E_INVOICE_API_LOG'] = 'debug';
      const client = new EInvoiceAPI({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['E_INVOICE_API_LOG'] = 'not a log level';
      const client = new EInvoiceAPI({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'E_INVOICE_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['E_INVOICE_API_LOG'] = 'debug';
      const client = new EInvoiceAPI({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new EInvoiceAPI({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['E_INVOICE_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new EInvoiceAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['E_INVOICE_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new EInvoiceAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['E_INVOICE_API_BASE_URL'] = ''; // empty
      const client = new EInvoiceAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });

    test('blank env variable', () => {
      process.env['E_INVOICE_API_BASE_URL'] = '  '; // blank
      const client = new EInvoiceAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new EInvoiceAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new EInvoiceAPI({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['E_INVOICE_API_API_KEY'] = 'My API Key';
    const client = new EInvoiceAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['E_INVOICE_API_API_KEY'] = 'another My API Key';
    const client = new EInvoiceAPI({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new EInvoiceAPI({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new EInvoiceAPI({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new EInvoiceAPI({ apiKey: 'My API Key', fetch: testFetch });
