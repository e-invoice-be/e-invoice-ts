...
import { APIPromise } from 'e-invoice-api/core/api-promise';

import util from 'node:util';
import EInvoice from 'e-invoice-api';
import { APIUserAbortError } from 'e-invoice-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['E_INVOICE_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: EInvoice) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new EInvoice({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new EInvoice({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new EInvoice({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['E_INVOICE_LOG'] = 'debug';
      const client = new EInvoice({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['E_INVOICE_LOG'] = 'not a log level';
      const client = new EInvoice({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'E_INVOICE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['E_INVOICE_LOG'] = 'debug';
      const client = new EInvoice({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new EInvoice({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['E_INVOICE_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new EInvoice({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['E_INVOICE_BASE_URL'] = 'https://example.com/from_env';
      const client = new EInvoice({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['E_INVOICE_BASE_URL'] = ''; // empty
      const client = new EInvoice({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.e-invoice.be');
    });

    test('blank env variable', () => {
      process.env['E_INVOICE_BASE_URL'] = '  '; // blank
      const client = new EInvoice({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.e-invoice.be');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new EInvoice({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['E_INVOICE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new EInvoice({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new EInvoice({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new EInvoice({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', () => {
      const client = new EInvoice({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new EInvoice({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['E_INVOICE_API_KEY'] = 'My API Key';
    const client = new EInvoice();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['E_INVOICE_API_KEY'] = 'another My API Key';
    const client = new EInvoice({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new EInvoice({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new EInvoice({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new EInvoice({ apiKey: 'My API Key', fetch: testFetch });
